...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const GhostknightMaxLevel = 80; // Максимальный уровень скелета
    let GhostknightLevel = Math.min(level, GhostknightMaxLevel); // Ограничение уровня скелета
    const GhostknightBaseHealth = 400; // Базовое здоровье скелета
    let GhostknightMaxHealth = GhostknightBaseHealth * GhostknightLevel;
    let GhostknightHealth = GhostknightMaxHealth;

    // Элементы интерфейса
    const GhostknightHealthBar = document.getElementById("ghostknight-health-fill");
    ...
    function updateGhostknightUI() {
        GhostknightHealthBar.style.width = `${(GhostknightHealth / GhostknightMaxHealth) * 100}%`;
    }
    ...
    function updateGhostknightHealth(damage) {
        GhostknightHealth -= damage;
        if (GhostknightHealth <= 0) {
            GhostknightHealth = 0;
            window.isMobAlive = false;
            updateGhostknightUI();

            gainExpFromMob(currentNickname, 'Ghostknight');
    ...
    attackSystem.performAttack = function () {
        ...
        console.log('Игрок наносит удар призрачному воину!');
        updateGhostknightHealth(attackSystem.playerStats.attackPower);
    ...
    function calculateReward() {
        const baseReward = 250; // Начальная награда
        const bonusPer10Levels = Math.floor(GhostknightLevel / 10) * 40; // Бонус за каждые 10 уровней
    ...
    function calculateLoot() {
        ...
        if (lootChance < 0.3) { // Шанс выпадения 40%
            const lootItemId = 104; // ID выпадающего предмета (Глаз гоблина)
    ...
    function showVictoryMessage(reward, lootItem = null) {
        ...
        message.style.background = "url('./assets/notification_plate_ghostknight.png') no-repeat center center";
    ...
    // Инициализация
    GhostknightMaxHealth = GhostknightBaseHealth * GhostknightLevel;
    GhostknightHealth = GhostknightMaxHealth;
    updatePlayerUI();
    updateGhostknightUI();
